...
SsdLegoSimLink::SsdLegoSimLink() {
  ...
  spdlog::info("[SsdLegoSimLink] enabled: self=({},{}) ssd_peer=({},{}) core_freq_mhz={}",
               _self_x, _self_y, _peer_x, _peer_y, _core_freq_mhz);
}

SsdLegoSimLink::RoundTrip SsdLegoSimLink::round_trip(const SsdLatencyRequest& req,
  ...
  // Response leg: peer -> self. ssd_simlet already folded its own modeled
  // latency into the cycle it wrote the response at, so the resolved cycle
  // interchiplet hands back here already reflects ssd_simlet's latency plus
...
uint64_t SsdLegoSimLink::query_latency_ns(uint64_t addr, uint64_t nbytes, uint64_t inst_id,
  ...
  if (_core_freq_mhz <= 0.0) {
    ...
    // normal simulator.py path, but guard anyway) -- fall back to
    // ssd_simlet's own modeled latency alone, ignoring transport delay,
  ...
  // acking) -- convert the elapsed cycles to ns for this method's contract;
  // DMA.cc converts back to cycles itself.
