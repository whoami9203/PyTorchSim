...
class SimStaticCache(StaticCache):
    ...
    run_tinyllama_gen_streamed_npu) so all layers share Dynamo guards and compile to a single
    reused kernel instead of separate specializations -- which means the `layer_idx` that
...
def _forward_streamed(model, loader, input_ids, attention_mask, past_key_values, device, dtype):
    ...
    # StaticCache returns a 0-dim tensor -- this call is eager (outside any torch.compile
    # boundary), so converting it here is a harmless host sync either way, not the traced-context
    # sync SimStaticCache's docstring warns about.
...
def _dump_module_weight_ranges(module, name_prefix):
    ...
    format/location run_tinyllama_gen already uses for the whole-model case) so
...
def _forward_streamed_npu(model, loader, prelude_fn, epilogue_fn, input_ids, attention_mask, past_key_values, device, dtype):
    ...
    base_model = model.model
    # See _forward_streamed's comment: eager-side int() is harmless for either cache class.
    past_seen_tokens = int(past_key_values.get_seq_length())
    cache_position = torch.arange(
    ...
    # See _forward_streamed: these two calls are SimStaticCache-only. HF's real StaticCache needs
    # neither -- update() gets cache_position from cache_kwargs and the real layer_idx from the
    # calling attention module directly.
    if hasattr(past_key_values, "set_write_offset"):
        past_key_values.set_write_offset(past_seen_tokens)
    for layer_idx, decoder_layer in enumerate(base_model.layers):
        loader.load_module_(decoder_layer, f"model.layers.{layer_idx}", device, dtype)
...
@torch.no_grad()
def run_tinyllama_gen_streamed_cpu(
    model_id="TinyLlama/TinyLlama-1.1B-Chat-v1.0",
    prompt="Hello!",
    dtype="float32",
    max_new_tokens=5,
    use_hf_static_cache=False,
):
    torch.manual_seed(0)
    print("\n[Running TinyLlama-1.1B streamed-layer CPU Test]")
...
@torch.no_grad()
def run_tinyllama_gen_streamed_npu(
    device,
    model_id="TinyLlama/TinyLlama-1.1B-Chat-v1.0",
    prompt="Hello!",
    dtype="float32",
    ...
    """Same layer-streamed loading as run_tinyllama_gen_streamed_cpu, but each decoder layer's
    ...
    torch.manual_seed(0)
    print("\n[Running TinyLlama-1.1B streamed-layer NPU Test]")
    dtype_map = {"float32": torch.float32, "float16": torch.float16, "bfloat16": torch.bfloat16}
    torch_dtype = dtype_map.get(dtype, torch.float32)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test TinyLlama (streamed weights, optional NPU sim)")
    ...
    parser.add_argument("--hf_static_cache", action="store_true",
                         help="Use transformers' real StaticCache instead of SimStaticCache -- "
                              "for checking whether the mark_static_address/index_copy_/"
                              "cache_position.item() issues SimStaticCache was built to avoid "
                              "(see its class docstring) still reproduce after later PyTorchSim changes")
    ...
    if args.npu:
        ...
        run_tinyllama_gen_streamed_npu(
