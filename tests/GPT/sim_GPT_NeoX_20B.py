...
from test_GPT_NeoX_20B import (
    ...
    _prelude,
    _epilogue,
    replace_linear_with_transposed_,
...
def _build_model_and_loader(model_id, dtype, device, num_layers):
    """Same meta-skeleton + streamed-checkpoint setup as test_GPT_NeoX_20B.py -- weights are still
    ...
    # NPU only: replace query_key_value/dense/dense_h_to_4h/dense_4h_to_h with
    ...
    # PyTorchSimFrontend/transposed_linear.py). Left as plain nn.Linear on CPU --
    # TransposedLinear computes the identical result either way, so this only changes
    # which path each device takes, not what either one computes.
    if device.type == "npu":
        replace_linear_with_transposed_(model.gpt_neox.layers)
        # Manual per-shape tile-size overrides -- see
        # configs/tile_overrides/gpt_neox_20b.yaml for what's in it and why. Only
        # takes effect if TOGSIM_CONFIG also has codegen_mapping_strategy:
        # external-then-heuristic (configs/eclab_cambricon_gpt_neox_tile_overrides.yml).
        apply_tile_overrides(TILE_OVERRIDE_YAML, TILE_OVERRIDE_JSON)

    # Rotary table is config-derived rather than checkpoint-derived (see _build_meta_model), and
    # comes out real-valued but CPU-resident -- move it onto the target device by hand, same as
    # sim_llama2_7B.py does for model.model.rotary_emb.
...
def _run_forward(model, loader, input_ids, attention_mask, past_key_values, device, torch_dtype, config, npu,
    ...
    if not use_togsimulator:
        ...
        # fresh TOGSim process per kernel) -- this is the default here since
        # that's this script's original behavior; pass use_togsimulator=True
        # (or --togsimulator on the CLI) for the persistent-process path
        # instead, same option as sim_llama2_7B.py's.
    ...
    # spawning a fresh TOGSim process per kernel -- see sim_llama2_7B.py's
    # identical option. extension_codecache.py's run_kernel_simulation only
    # takes the persistent-process path when torch.npu.get_tog_simulator()
    # is non-None, which requires an active TOGSimulator context. Covers
    # run_prefill/run_decode/run_compare alike, since they all call through
    # here for their NPU dispatch.
...
def _kv_head_shape(config):
    """GPTNeoXConfig has no num_key_value_heads/head_dim fields at all (unlike LlamaConfig, where
    they exist but may be None) -- GPT-NeoX uses plain multi-head attention, so both fall back to
    the standard hidden_size/num_attention_heads split."""
...
@torch.no_grad()
def run_compare(model_id, phase, length, batch, dtype, num_layers, seed, rtol=1e-3, atol=1e-3, use_togsimulator=False):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Simulate a GPT-NeoX-20B prefill or decode phase in isolation. Token ids and "
    ...
    parser.add_argument("--hf_model", type=str, default="EleutherAI/gpt-neox-20b")
